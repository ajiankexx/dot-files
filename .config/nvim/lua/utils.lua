...
-- Create a pair of repeatable move functions (forward/backward)
-- Backward-compatible replacement for the removed make_repeatable_move_pair
function M.make_repeatable_move_pair(forward_fn, backward_fn)
    local ok, ts_repeat_move = pcall(require, "nvim-treesitter-textobjects.repeatable_move")
    if not ok then return forward_fn, backward_fn end
    if ts_repeat_move.make_repeatable_move_pair then
        return ts_repeat_move.make_repeatable_move_pair(forward_fn, backward_fn)
    end
    local wrapped = ts_repeat_move.make_repeatable_move(function(opts)
        if opts.forward then forward_fn() else backward_fn() end
    end)
    return function() wrapped({ forward = true }) end,
        function() wrapped({ forward = false }) end
end
return M
